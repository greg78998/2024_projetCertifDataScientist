...
X6_construction_base <- function(forme_dt_ls,para_succ_nb_periode, para_dt_placement = dt_placement){
  
  if (any(grepl("simple", forme_dt_ls))){
    DB <- readRDS(paste0(path_data_vf,"/",para_dt_placement,"_DB_postRET.RDS"))
    print("La table contient les variables simples")
  }

  if (any(grepl("add_surplus", forme_dt_ls))){
    DB_surplus <- readRDS(paste0(path_data_vf,"/",para_dt_placement,"_top_surplus.RDS"))
  ...
  if (any(grepl("add_chocs", forme_dt_ls))){
    DB_chocs <- readRDS(paste0(path_data_vf,"/",para_dt_placement,"_top_chocs.RDS"))
  ...
  if (any(grepl("add_succ_surplus", forme_dt_ls))){
    DB_succ_surplus <- readRDS(paste0(path_data_vf,"/",para_dt_placement,"_succ_",para_succ_nb_periode,"_surplus.RDS"))
  ...
  if (any(grepl("add_succ_chocs", forme_dt_ls))){
    DB_succ_chocs <- readRDS(paste0(path_data_vf,"/",para_dt_placement,"_succ_",para_succ_nb_periode,"_chocs.RDS"))
  ...
  if (any(grepl("poly", forme_dt_ls))){
    DB_poly <- readRDS(paste0(path_data_vf,"/",para_dt_placement,"_sq_rf_M.RDS"))
    DB <- DB %>% cbind(DB_poly)
